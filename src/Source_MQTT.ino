...
// ******************************************************
void UpdatePmqtt() {
  // Si un "label" de canal est configuré (LabelP), la mesure Maison est extraite directement dans le
  // callback() MQTT (EnvoiMQTT.ino) depuis un message multi-canaux : PuissanceS_M_inst/PuissanceI_M_inst,
  // Energie_M_Soutiree/Injectee etc. sont déjà à jour, pas besoin du calcul Pw/Pva/Pf ci-dessous.
  if (LabelP.length() == 0) {
  ...
  // La mesure Triac (si un topic MQTT dédié TopicIT est configuré) est traitée directement dans le
  // callback() MQTT (EnvoiMQTT.ino), pas ici : elle arrive sur un topic séparé, indépendant de Source.

  filtre_puissance();

  if (LabelP.length() > 0 || P_MQTT_Brute.indexOf("Pw") > 0) EnergieActiveValide = true;
